...
def doc():
    ...
    return ParsedDocument(units=units, root_tag="CONS.ACT")


class TestPacking:
    def test_short_sibling_paragraphs_are_packed_together(self, doc, counter):
        drafts = chunk_document(doc, TITLE, counter)
    ...
    def test_packed_chunks_attach_to_the_article_not_the_paragraph(self, doc, counter):
        drafts = chunk_document(doc, TITLE, counter)
    ...
    def test_packing_never_crosses_an_article_boundary(self, doc, counter):
        # A chunk spanning two articles could not be cited precisely.
        for draft in chunk_document(doc, TITLE, counter):
            assert "delta" not in draft.body or "alpha" not in draft.body

    def test_an_oversized_paragraph_is_split_rather_than_dropped(self, doc, counter):
        drafts = chunk_document(doc, TITLE, counter)
    ...
    def test_every_chunk_fits_the_embedding_input_limit(self, doc, counter):
        for draft in chunk_document(doc, TITLE, counter):
            assert draft.token_count < settings.embed_input_token_limit

    def test_annexes_and_recitals_are_still_chunked(self, doc, counter):
        paths = {d.unit_path for d in chunk_document(doc, TITLE, counter)}
        assert "ANX_III" in paths and "RCT_27" in paths

    def test_chapters_are_not_embedded(self, doc, counter):
...
class TestUnitTypeFilter:
    def test_recitals_only_excludes_operative_text(self, doc, counter):
        drafts = chunk_document(doc, TITLE, counter, frozenset({"recital"}))
        assert {d.unit_path for d in drafts} == {"RCT_27"}

    def test_filtering_out_paragraphs_falls_back_to_article_text(self, doc, counter):
        drafts = chunk_document(doc, TITLE, counter, frozenset({"article"}))
        art6 = [d for d in drafts if d.unit_path == "CH_III/ART_6"]
...
class TestBreadcrumbs:
    ...
    def test_breadcrumb_is_included_in_the_embedded_text(self, doc, counter):
        drafts = chunk_document(doc, TITLE, counter)
        art6 = next(d for d in drafts if d.unit_path == "CH_III/ART_6")
        assert art6.text.startswith(TITLE)
        assert art6.body in art6.text

    def test_body_excludes_the_breadcrumb(self, doc, counter):
...
class TestTokenCounting:
    def test_counts_grow_with_text(self, counter):
