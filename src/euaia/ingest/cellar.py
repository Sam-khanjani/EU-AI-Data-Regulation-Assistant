"""Client for the EU Publications Office CELLAR repository, and links into EUR-Lex.
...
* **Deep links** into EUR-Lex's rendered edition, so a reader can check a citation at its
  source (:func:`deeplink`).
...
# ------------------------------------------------------------------- deep links
# Deep links into EUR-Lex for citable units.
#
...
def _anchor_for(unit_type: str, unit_number: str | None, unit_path: str = "") -> str | None:
...
def deeplink(celex: str, unit_type: str, unit_number: str | None, unit_path: str = "") -> str:
    """Full EUR-Lex URL for a unit, anchored where possible."""
    url = f"{EURLEX_BASE}?uri=CELEX%3A{quote(celex)}"
    fragment = _anchor_for(unit_type, unit_number, unit_path)
